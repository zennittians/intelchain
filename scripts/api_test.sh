...
while getopts "lbvp" OPTION; do
	...
	b)
		NETWORK="betanet"
		declare -A PORT=( [POST]="http://s0.b.intelchain.org:9500/" [GET]="http://e0.b.intelchain.org:5000/" )
		BLOCK_0_HASH=$(curl --location --request POST "http://l0.b.intelchain.org:9500" \
		...
		TRANSACTION_HASH=$(curl  --location --request POST "http://l0.b.intelchain.org:9500" \
			  --header "Content-Type: application/json" \
			    --data "{\"jsonrpc\":\"2.0\",\"method\":\"itc_sendRawTransaction\",\"params\":[\""$SIGNED_RAW_TRANSACTION"\"],\"id\":1}" | jq -r '.result')
		echo "TRANSACTION_HASH:"
		echo $TRANSACTION_HASH
		sleep 20s
		TRANSACTION=$(curl --location --request POST "http://l0.b.intelchain.org:9500" \
			  --header "Content-Type: application/json" \
